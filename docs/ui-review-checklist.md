...
- 今日完成态应出现 `课程反馈`，并展示 `太简单 / 刚好 / 太难`、`有帮助 / 一般 / 没帮助`、`想深入 / 跳过类似主题` 三组反馈信号；未完成态不应出现该反馈区
- 今日完成态应出现 `推荐语音反思`，包含 `60 秒`提示，并链接到 `/voice?lessonId=...&mode=daily_understanding`
- 今日完成态如存在 active book session，应出现 `关联阅读`，展示 `《AI Engineering》第 12-14 页可以补充今天的主题。`，并提供 `/books/ai-engineering` 的 `去同读` 入口
- 今日完成后如出现项目实践区，应显示 `下一步：把今天的知识用到项目里` 和 `推荐项目任务`
- 今日完成后 `推荐语音反思`、`关联阅读`、项目实践和下一步 action CTA 在手机端应是全宽大触控目标，至少包含 `min-h-11` 和 `w-full sm:w-auto`
...
- 如果本轮 `forgot + hard` 多于 `good + easy`，完成总结应明确提示 `今天先不要学新内容，建议复习和修复。`
- 如果本轮出现 `forgot` 或 `hard`，应看到 `补弱动作`，包含 `让 Coach 解释这些卡片`、`生成补弱小课`、`明天安排补弱`、`查看错题中心`
- `明天安排补弱` 应链接到带 `when=tomorrow` 的 `/today?mode=remediation&source=review...`，避免用户今天继续开新内容
...
- 桌面侧边栏应按学习者心智分组显示 `今日`、`学习动作`、`知识资产`、`系统`，不应继续显示旧分组 `学习主线`、`补弱与表达`、`知识与探索`
- 桌面 `今日` 分组应包含首页、今日任务、复习中心、当前路径、每周复盘；`学习动作` 应包含 Coach、说出理解、错题误区、项目任务、同读书籍；`知识资产` 应包含知识地图、课程库、我的笔记、术语路径、AI Radar、学习进度
...
- 页面应显示 `阶段地图`，把 7 个学习阶段渲染成节点式路线，而不是只显示长列表报告
- `阶段地图` 节点状态应使用中文 `完成`、`当前`、`锁定`、`补弱`，不应只靠颜色表达阶段状态
- `阶段地图` 中有阻塞且未完成的阶段应突出 `补弱` 和 `当前阻塞`
...
- 每个阶段应显示 `下一步主题`
- 每张阶段卡默认只显示阶段名、进度、完成标准、当前阻塞、解锁条件和下一步；`当前信号`、为什么要过这一阶段和 criteria 明细应放在 `详细指标` 折叠区内
- 与 Books 相关的阶段应显示 `阶段阅读`，包含书名、页码范围、推荐理由和 `/books/:id` 的 `去同读` 入口
- `阶段阅读` 的 `去同读` CTA 在手机端应是全宽大触控目标，至少包含 `min-h-11` 和 `w-full sm:w-auto`
...
- 每条误区和聚焦误区的修复动作应包含 `做一道同类题`，并链接到 `/today?mode=remediation&source=mistake...` 的短练习落点
- 从 `做一道同类题` 进入 `/today?mode=remediation&source=mistake...` 时，应显示 `Mistake 同类题短练习`、`错题修复中心`、`同类题已带入` 和 `生成同类题短练习`
...
- 顶部 `当前任务` 后应能看到 `本周学习总结`，包含 `本周称号`、一句本周总结和称号原因
- `本周学习总结` 应汇总学习天数、完成课程、复习卡片和修复误区，例如 `你学习了 N 天，完成 N 节课，复习 N 张卡，修复 N 个误区。`
- `周记` 表单应可见，默认内容或提示应包含 `我这周最大的收获是...` 和 `我下周想重点学...`
...
- 能看到 `本周最值得修复的 3 个误区`，不应回退到旧的单条 `错题最多的概念` 卡片
- `本周最值得修复的 3 个误区` 每条应链接到 `/mistakes?focus=<id>`，并显示中文来源、出现次数和关联课程
- `本周最值得修复的 3 个误区` 每个入口在手机端应满足 44px 触控高度，至少包含 `min-h-11`
