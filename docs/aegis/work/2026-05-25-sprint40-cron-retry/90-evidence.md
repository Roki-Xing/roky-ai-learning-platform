...
- RED：`npm test -- tests/unit/cron-daily.test.ts` 失败，错误为 `retryFailedDailyCronJob is not a function`。
- GREEN：`npm test -- tests/unit/cron-daily.test.ts` 7 项通过。
- 本地：`npm run lint` 通过。
- 本地：`npm test` 120 项通过。
- 本地：`npm run build` 通过，路由表包含 `/admin` 和 `/api/cron/daily`。

## 生产验证

- 备用机远端代码备份：`/home/ubuntu/deploy-backups/ai-learning-platform-before-sprint40-20260525-042846.tar.gz`。
- 已通过 `rsync` 同步到备用机 `118.25.15.72:/home/ubuntu/ai-learning-platform`，同步排除 `.git/`、`node_modules/`、`.next/`、`.env`、`.env.*`。
- 备用机远端目标测试：`npm test -- tests/unit/cron-daily.test.ts` 7 项通过。
- 备用机远端构建：`npm run build` 通过，路由表包含 `/admin` 和 `/api/cron/daily`。
