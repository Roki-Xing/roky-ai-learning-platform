...
- [√] 运行本地完整门禁。
- [√] 提交并推送 GitHub：`3a4e891 feat: reorganize desktop navigation intent groups`。
- [√] 部署到 `118.25.15.72` 的 `ai-learning-platform` 容器，并完成 `learn.roky.chat` 生产 smoke。
- [√] 补生产部署证据并清理临时 SSH key / 登录态临时文件。

## Current Resume State Hint

从 `ai-learning-platform` 根目录继续。当前切片是 `0.361.0 Reduce Chaos Desktop Navigation Intent Groups`：只改桌面导航分组数据、导航相关测试和文档记录；不要改移动底部导航主入口、More Sheet 行为、认证策略、Preview 写保护、数据库 schema、生产 env/provider 密钥或页面业务流程。已完成 RED/GREEN、相关回归、route/learning 审计、完整本地门禁、GitHub push、生产部署、远端门禁、生产 health、桌面导航只读 smoke 和临时 SSH key / 登录态文件清理。生产 smoke 只检查桌面侧边栏可见 `今日`、`学习动作` 和 `知识资产`，未做写入型操作。
...
- Decision：complete; RED/GREEN、相关回归、审计、完整本地门禁、GitHub push、生产部署、远端门禁、生产 health、桌面导航只读 smoke 和临时 SSH key / 登录态文件清理均已完成。完整 Playwright 移动端截图矩阵、写入型生产 smoke、数据库迁移、路由保护变更、Preview 写保护变更、移动底部导航重构和既有 `npm audit` moderate 告警保持未覆盖边界。
...
本地 Reduce Chaos guidance 继续推进第 3.3、第 4.2、第 14.5 和第 20.9 节：Current Mission Reading Step。`src/server/learning/current-mission.ts` 将首页 `当前任务` 的 `今日闭环` 从 5 步扩展为 6 步，新增独立 `阅读` 步骤；当 active book session 存在且主课、复习、表达和修复前置任务清空时，`阅读` 成为 current，`实践` 保留为项目任务步骤。本切片只改读侧 Current Mission 进度模型、任务卡布局、源码级测试和文档记录；代码层不触碰数据库、Next Best Action 优先级、Books 静态数据、真实上传/OCR/AI provider、Preview 写保护、生产配置、生产密钥或迁移。生产上线只同步已验证读侧产物并执行只读 smoke。

- `npm test -- tests/unit/current-mission.test.ts`：RED 首次失败于 `CurrentMissionProgress` 仍为 5 步；GREEN 后 13 项通过，覆盖 `阅读` 单独步骤、active book session 时 `阅读:current` 和任务卡六步 `aria-label`。
- `npm test -- tests/unit/current-mission.test.ts tests/unit/next-best-action.test.ts tests/unit/home-page-labels.test.ts tests/unit/learning-ui-components.test.ts tests/unit/books-companion.test.ts tests/unit/learning-motivation.test.ts`：相关回归 71 项通过，覆盖 Current Mission、Next Best Action、首页、Books、学习动机和共享学习 UI。
- `git diff --check`、`npm run audit:routes`、`npm run audit:learning`、`npm run lint`、全量 `npm test`、`npm run build`：本地最终门禁通过；全量单测 475 项通过，Next 生产构建生成 31 个静态页面。
- `git push origin main`：代码提交 `0b18daf feat: add current mission reading step / 添加当前任务阅读步骤` 已推送到 GitHub `origin/main`。
- 生产备份、同步和重启已完成：`/home/ubuntu/deploy-backups/ai-learning-platform-before-0.367.0-20260610-122627.tar.gz`，`rsync` 到 `118.25.15.72:/home/ubuntu/ai-learning-platform`，并重启 `ai-learning-platform` 容器。
- 远端容器门禁通过：在生产容器 `node:22-bookworm` 内执行 `npm ci --include=dev`、`npm run prisma:generate`、Current Mission / Home / Books / Learning UI 相关回归 71 项、`npm run audit:routes`、`npm run audit:learning`、`npm run lint`、`npm run build`，随后 `npm prune --omit=dev`。
- 生产只读验收通过：公网 `https://learn.roky.chat/api/health` 返回 200/ok；容器内 `127.0.0.1:3102/api/health` 返回 200/ok；390px 登录态首页 smoke 可见 `Roky Learn`、`当前任务`、`今日闭环`、`学习`、`复习`、`表达`、`修复`、`实践` 和 `阅读`。
- 远端产物检查通过：`.next/server` 中确认 Current Mission 卡片产物包含 `grid-cols-6`，首页产物包含 `今日闭环`，服务端构建产物包含 `阅读`。
- 临时文件清理完成：已删除本地 `/tmp/roky_deploy_0367.pem`、`/tmp/roky_known_hosts_0367`，并清理远端 `/tmp/roky-*0367*` smoke 临时文件。
- `python3 /home/xing-12_26/projects/codex-workspace/CodeShua/references/Aegis/scripts/aegis-workspace.py bundle --root /home/xing-12_26/projects/codex-workspace/ai-learning-platform --work 2026-06-03-roky-learning-desire`：仍失败于既有 Aegis Markdown-only 结构债，缺 `task-intent-draft.json`。该结果归类为方法包结构债，不是产品 UI 验证失败。
- `python3 /home/xing-12_26/projects/codex-workspace/CodeShua/references/Aegis/scripts/aegis-workspace.py check --root /home/xing-12_26/projects/codex-workspace/ai-learning-platform`：仍失败于当前和多个既有 work markdown 未索引。该结果归类为方法包结构债，不是产品 UI 验证失败。

## Resume State Hint

从 `ai-learning-platform` 根目录继续。不要从记忆单独恢复；先读本文件、`10-intent.md`、`90-evidence.md`，再检查 `git status`。当前 Reduce Chaos Current Mission Reading Step 已完成 RED/GREEN、相关回归、`git diff --check`、路由审计、学习审计、lint、全量单测、build、GitHub push、生产备份/同步/重启、远端容器门禁、生产 health、390px 登录态首页只读 smoke、远端产物检查和临时敏感文件清理；Aegis helper 失败仍归类为既有 Markdown-only 结构债，不是产品 UI 验证失败。本轮只改读侧 Current Mission 进度模型、任务卡布局、源码级测试和文档，不触碰数据库、写路径、生产配置或密钥。

## Drift Check

- Scope：仍服务 Roky Learn reduce chaos、Book Companion 和 Daily Command Center 目标，当前切片让用户在首页明确看到 `阅读` 是每日学习闭环的一步。
- Compatibility：不新增迁移，不触碰 Next Best Action 排序、Books 数据源、Current Mission 卡片 CTA、上传禁用态、Preview Mode 只读边界或数据库；只改变进度模型展示层和测试。
- New fallback/owner：未新增 owner、adapter 或外部 fallback；Current Mission 进度 owner 仍为 `src/server/learning/current-mission.ts` 和 `CurrentMissionCard`。
- Retirement：旧的 `实践` 兼容 Books mixed step 已退役；Books active reading 不再把 `实践` 标为 current。
- Decision：complete; Reduce Chaos Current Mission Reading Step 已完成产品级 RED/GREEN、本地相关回归、完整本地门禁、GitHub push、生产备份/同步/重启、远端容器门禁、生产 health、390px 登录态首页只读 smoke、远端产物检查和临时敏感文件清理。完整 Playwright 移动端截图矩阵、真实移动设备 smoke、写入型生产 smoke、数据库迁移、真实 PDF 上传、OCR、AI provider 调用、课程反馈持久化、Curriculum Planner 写入和既有 `npm audit` moderate 告警保持未覆盖边界。Aegis helper 仍失败于既有 Markdown-only 结构债，归类为方法包结构债，不是产品 UI 验证失败。

## Reduce Chaos Book Chapter Weekly Recap

本地 Reduce Chaos guidance 继续推进第 9.3 和第 14.2 节：Book Chapter Weekly Recap。`src/server/learning/weekly.ts` 将 active book session 转为 `weeklyBookChapters`，`/weekly` 显示 `本周同读章节`，导出的 Weekly Markdown 也包含同读章节小节。本切片只改读侧 Weekly 数据合约、页面展示、源码级测试和文档记录，不触碰数据库、Books 静态数据、读书进度写入、OCR、AI provider、Preview 写保护、生产/SSH/部署/密钥。

- `npm test -- tests/unit/weekly-review.test.ts`：RED 首次失败于 `snapshot.weeklyBookChapters` 为 `undefined`；GREEN 后 10 项通过，覆盖 `weeklyBookChapters`、`本周同读章节`、`AI Engineering`、`第 12-14 页`、`/books/ai-engineering`、Weekly Markdown 小节和手机端 `weeklyBookChapterLinkClassName`。
- `npm test -- tests/unit/weekly-review.test.ts tests/unit/books-companion.test.ts tests/unit/current-mission.test.ts tests/unit/learning-path.test.ts tests/unit/learning-ui-components.test.ts`：相关回归 57 项通过，覆盖 Weekly、Books、Current Mission、Path 和共享学习 UI。
- `rg -n "Book Chapter Weekly Recap|weeklyBookChapters|weeklyBookChapterLinkClassName|本周同读章节|AI Engineering|第 12-14 页|/books/ai-engineering|0\\.368\\.0" ...`：覆盖扫描确认源码、测试、UI checklist、Weekly/Books 模块文档、CHANGELOG 和 Aegis 记录均接入本切片。
- `git diff --check`、`npm run audit:routes`、`npm run audit:learning`、`npm run lint`、全量 `npm test`、`npm run build`：本地最终门禁通过；全量单测 476 项通过，Next 生产构建生成 31 个静态页面。
- `python3 /home/xing-12_26/projects/codex-workspace/CodeShua/references/Aegis/scripts/aegis-workspace.py bundle --root /home/xing-12_26/projects/codex-workspace/ai-learning-platform --work 2026-06-03-roky-learning-desire`：仍失败于既有 Aegis Markdown-only 结构债，缺 `task-intent-draft.json`。该结果归类为方法包结构债，不是产品 UI 验证失败。
- `python3 /home/xing-12_26/projects/codex-workspace/CodeShua/references/Aegis/scripts/aegis-workspace.py check --root /home/xing-12_26/projects/codex-workspace/ai-learning-platform`：仍失败于当前和多个既有 work markdown 未索引。该结果归类为方法包结构债，不是产品 UI 验证失败。

## Resume State Hint

从 `ai-learning-platform` 根目录继续。不要从记忆单独恢复；先读本文件、`10-intent.md`、`90-evidence.md`，再检查 `git status`。当前 Reduce Chaos Book Chapter Weekly Recap 已完成 RED/GREEN、相关回归、覆盖扫描、路由审计、学习审计、lint、全量单测、build 和 Aegis helper 结构检查；Aegis helper 失败仍归类为既有 Markdown-only 结构债，不是产品 UI 验证失败。尚未提交、推送或生产部署。本轮只改读侧 Weekly 数据合约、页面展示、源码级测试和文档，不触碰数据库、写路径、生产配置或密钥。

## Drift Check

- Scope：仍服务 Roky Learn reduce chaos、Book Companion 和 Weekly ritual 目标，当前切片让 `Book chapter → Weekly` 从静态连接变成周复盘可见内容。
- Compatibility：不新增迁移，不触碰 Books 数据源、Current Mission 排序、上传禁用态、Preview Mode 只读边界、数据库或生产配置；只新增 Weekly 读侧字段和展示。
- New fallback/owner：未新增外部 owner、adapter 或兼容 fallback；Weekly 同读章节 owner 为 `src/server/learning/weekly.ts` 和 `src/app/weekly/page.tsx`。
- Retirement：旧的 Weekly 不显示读书章节，导致 Books 读书进度无法进入周复盘；该断点已由 `weeklyBookChapters` 和 `本周同读章节` 卡片退役。
- Decision：continue; Reduce Chaos Book Chapter Weekly Recap 已完成产品级 RED/GREEN、相关回归、覆盖扫描、完整本地门禁和 Aegis helper 结构检查。下一步提交和推送；生产部署、生产 smoke、数据库迁移、真实 PDF 上传、OCR、AI provider 调用和读书进度持久化保持未覆盖边界。

## Reduce Chaos Today Related Reading Handoff

本地 Reduce Chaos guidance 继续推进第 5.4 节：Today 与 Books 联动。`src/server/learning/today-completion-actions.ts` 新增 `relatedReading`，`src/app/today/page.tsx` 将 active book session 传入完成后行动规则，`LearningCompletionCard` 在今日完成态显示 `关联阅读`、`《AI Engineering》第 12-14 页可以补充今天的主题。` 和 `/books/ai-engineering` 的 `去同读` 入口。本切片只改读侧 Today 完成态、Books active session 接线、源码级测试和文档记录，不触碰数据库、Books 静态数据、读书进度写入、OCR、AI provider、Preview 写保护、生产/SSH/部署/密钥。

- `npm test -- tests/unit/today-completion-next-actions.test.ts`：RED 首次失败于完成卡缺少 `关联阅读`；GREEN 后 12 项通过，覆盖 active book session、`AI Engineering`、`第 12-14 页`、`去同读`、`/books/ai-engineering` 和手机端 CTA 触控 class。
- `npm test -- tests/unit/today-completion-next-actions.test.ts tests/unit/books-companion.test.ts tests/unit/current-mission.test.ts tests/unit/learning-path.test.ts tests/unit/weekly-review.test.ts tests/unit/learning-ui-components.test.ts`：相关回归 69 项通过，覆盖 Today、Books、Current Mission、Path、Weekly 和共享学习 UI。
- `rg -n "Today Related Reading Handoff|relatedReading|关联阅读|去同读|AI Engineering|第 12-14 页|/books/ai-engineering|0\\.369\\.0" ...`：覆盖扫描确认源码、测试、UI checklist、Today/Books 模块文档、CHANGELOG 和 Aegis 记录均接入本切片。
- `git diff --check`、`npm run audit:routes`、`npm run audit:learning`、`npm run lint`、全量 `npm test`、`npm run build`：本地最终门禁通过；全量单测 477 项通过，Next 生产构建生成 31 个静态页面。
- `python3 /home/xing-12_26/projects/codex-workspace/CodeShua/references/Aegis/scripts/aegis-workspace.py bundle --root /home/xing-12_26/projects/codex-workspace/ai-learning-platform --work 2026-06-03-roky-learning-desire`：仍失败于既有 Aegis Markdown-only 结构债，缺 `task-intent-draft.json`。该结果归类为方法包结构债，不是产品 UI 验证失败。
- `python3 /home/xing-12_26/projects/codex-workspace/CodeShua/references/Aegis/scripts/aegis-workspace.py check --root /home/xing-12_26/projects/codex-workspace/ai-learning-platform`：仍失败于当前和多个既有 work markdown 未索引。该结果归类为方法包结构债，不是产品 UI 验证失败。

## Resume State Hint

从 `ai-learning-platform` 根目录继续。不要从记忆单独恢复；先读本文件、`10-intent.md`、`90-evidence.md`，再检查 `git status`。当前 Reduce Chaos Today Related Reading Handoff 已完成 RED/GREEN、相关回归、覆盖扫描、文档同步、本地完整门禁和 Aegis helper 结构检查；Aegis helper 失败仍归类为既有 Markdown-only 结构债，不是产品 UI 验证失败。尚未提交或推送。本轮只改读侧 Today 完成态、Books active session 接线、源码级测试和文档，不触碰数据库、写路径、生产配置或密钥。

## Drift Check

- Scope：仍服务 Roky Learn reduce chaos、Book Companion 和 Today completion flow 目标，当前切片让 `读相关书籍页` 从指导文档建议变成完成卡里的真实后续动作。
- Compatibility：不新增迁移，不触碰 Books 数据源、Current Mission 排序、上传禁用态、Preview Mode 只读边界、数据库或生产配置；只新增 Today 完成态读侧字段和展示。
- New fallback/owner：未新增外部 owner、adapter 或兼容 fallback；Today 完成态关联阅读 owner 为 `src/server/learning/today-completion-actions.ts`、`src/components/learning/learning-completion-card.tsx` 和 `src/app/today/page.tsx`。
- Retirement：旧的 Today 完成态缺少 Books 关联阅读，导致今日主题和同读书籍之间仍断开；该断点已由 `relatedReading` 和 `关联阅读` 卡片退役。
- Decision：continue; Reduce Chaos Today Related Reading Handoff 已完成产品级 RED/GREEN、相关回归、覆盖扫描、文档同步、本地完整门禁和 Aegis helper 结构检查。下一步提交推送；生产部署、生产 smoke、数据库迁移、真实 PDF 上传、OCR、AI provider 调用和读书进度持久化保持未覆盖边界。

## Reduce Chaos Review Tomorrow Remediation Action

本地 Reduce Chaos guidance 继续推进第 6.3 节：Review 修复动作。`src/server/review/session-summary.ts` 在弱复习场景的 `remediationActions` 中新增 `明天安排补弱`，链接到带 `when=tomorrow` 的 `/today?mode=remediation&source=review...`；`src/app/review/ui/review-trainer.tsx` 将补弱动作区调整为中等屏两列、宽屏四列，承接四个修复动作。本切片只改读侧 Review 完成态、Today remediation 链接参数、源码级测试和文档记录，不触碰数据库、Review 评分排期、队列过滤、Today remediation 写入流程、认证策略、Preview 写保护、生产配置或密钥。

- [√] 读取 `roky_learn_reduce_chaos_and_book_companion_guidance.md`，确认当前切片对应第 6.3：Review 完成后修复动作应包含 `让 Coach 解释`、`加入 Mistakes`、`生成补弱小课` 和 `明天安排补弱`。
- [√] RED：扩展 `tests/unit/review-session-summary.test.ts`，先失败于 `remediationActions` 缺少 `明天安排补弱`。
- [√] GREEN：`buildRemediationActions()` 新增 `明天安排补弱`，并通过 `when=tomorrow` 参数把 Review 补弱接到次日 Today。
- [√] GREEN：`ReviewTrainer` 四个补弱动作布局调整为 `sm:grid-cols-2 xl:grid-cols-4`，避免三列布局容纳四动作时拥挤。
- [√] 运行定向 RED/GREEN。
- [√] 同步 `docs/ui-review-checklist.md`、`helloagents/modules/review.md`、`helloagents/CHANGELOG.md` 初始记录。
- [√] 运行覆盖扫描。
- [√] 运行相关回归。
...
从 `ai-learning-platform` 根目录继续。当前切片是 `0.370.0 Reduce Chaos Review Tomorrow Remediation Action`：只改 `/review` 完成态补弱动作、Today remediation 次日链接参数、源码级测试和文档记录；不要改 Review 评分排期、队列过滤、数据库 schema/migration、认证策略、Preview 写保护、生产 env/provider 密钥、Today remediation 写入流程或生产部署。已完成 RED/GREEN、覆盖扫描、相关回归、本地完整门禁和 Aegis helper 结构检查；Aegis helper 失败仍归类为既有 Markdown-only 结构债，不是产品 UI 验证失败。下一步提交并推送 GitHub。

## Drift Check

- Scope：仍服务 Roky Learn reduce-chaos 主线，把 `/review` 完成态从三类立即修复动作推进为包含次日补弱安排的完整修复闭环。
- Compatibility：不新增迁移，不触碰 Review 排期、队列过滤、Preview Mode 只读边界、数据库或生产配置；只新增读侧 action 和链接参数。
- New fallback/owner：未新增外部 owner、adapter 或兼容 fallback；Review 完成总结 owner 仍为 `src/server/review/session-summary.ts` 和 `ReviewTrainer`。
- Retirement：旧的 Review 弱项修复动作缺少 `明天安排补弱`，指导文件第 6.3 的第四个动作不可点击；该断点由新增 `when=tomorrow` Today remediation 链接退役。
- Decision：continue; Reduce Chaos Review Tomorrow Remediation Action 已完成 RED/GREEN、覆盖扫描、相关回归、完整本地门禁和 Aegis helper 结构检查。下一步提交和推送；生产部署、生产 smoke、数据库迁移、Review 排期变更和写入型生产 smoke 保持未覆盖边界。

## Reduce Chaos Mistakes Repair Workflow Progress

本地 Reduce Chaos guidance 继续推进第 7.1 和第 7.2 节：Mistakes 修复流程。`src/server/mistakes/view.ts` 新增 `buildMistakeRepairWorkflow()`，用现有 `Misconception.status`、`mistake-card:{id}` 复习卡和 `reviewCount` 推导 `发现误区 / 让 Coach 解释 / 生成复习卡 / 完成一次复习 / 标记已解决`；`src/app/mistakes/page.tsx` 在聚焦误区和每条误区中显示 `修复流程`。本切片只改读侧流程展示、现有卡片查询、源码级测试和文档记录，不触碰数据库迁移、Preview 写保护、写入 action、Current Mission 排序、Review 评分或生产配置。

- [√] 读取 `roky_learn_reduce_chaos_and_book_companion_guidance.md`，确认当前切片对应第 7.1 / 7.2：Mistakes 应呈现 `open -> explained -> card_created -> reviewed -> resolved` 修复流程。
- [√] RED：扩展 `tests/unit/mistakes-view.test.ts`，先失败于缺少 `buildMistakeRepairWorkflow()` 和页面 `修复流程` 接线。
- [√] GREEN：新增五步读侧流程推导，open/active/card_created/reviewed/resolved 均有 `已完成 / 当前 / 待办` 状态。
- [√] GREEN：`/mistakes` 聚焦误区和误区清单展示同一套 `修复流程`。
- [√] 运行定向 RED/GREEN。
- [√] 运行相关回归。
- [√] 同步 `docs/ui-review-checklist.md`、`helloagents/modules/mistakes.md`、`helloagents/CHANGELOG.md` 初始记录。
- [√] 运行覆盖扫描。
- [√] 运行本地完整门禁。
- [√] 运行 Aegis helper 结构检查并记录既有 Markdown-only 结构债。
- [ ] 提交并推送 GitHub。
...
从 `ai-learning-platform` 根目录继续。当前切片 `0.371.0 Reduce Chaos Mistakes Repair Workflow Progress` 已提交并推送到 GitHub：`9536d7e feat: show mistake repair workflow progress / 显示错题修复流程进度`。该切片只改 `/mistakes` 读侧修复流程展示、现有 `Flashcard.reviewCount` 查询、源码级测试和文档；未改数据库 schema/migration、`generateMistakeReviewCardAction()`、`markMistakeResolvedAction()`、Review 评分、Current Mission 排序、Preview 写保护、生产 env/provider 密钥或生产部署。

## Drift Check

- Scope：仍服务 Roky Learn reduce-chaos 主线，把 `/mistakes` 从错题列表和动作按钮推进为可理解的修复流程。
- Compatibility：不新增迁移，不触碰 Preview Mode 只读边界、写入 action、Review 评分、Current Mission 排序、数据库 schema 或生产配置；只用现有 `Misconception.status` 和 `Flashcard.reviewCount` 做读侧推导。
- New fallback/owner：未新增外部 owner、adapter 或兼容 fallback；Mistakes 修复流程 owner 为 `src/server/mistakes/view.ts` 和 `src/app/mistakes/page.tsx`。
- Retirement：旧的 `/mistakes` 只展示 `让 Coach 解释 / 生成复习卡 / 标记已解决` 动作，用户不知道处于修复流程哪一步；该断点由 `buildMistakeRepairWorkflow()` 和页面 `修复流程` 展示退役。
- Decision：complete; Reduce Chaos Mistakes Repair Workflow Progress 已完成 RED/GREEN、覆盖扫描、相关回归、完整本地门禁、Aegis helper 结构检查、GitHub push 和远端 HEAD 校验。生产部署、生产 smoke、数据库迁移和写入型生产 smoke 保持未覆盖边界。Aegis helper 仍失败于既有 Markdown-only 结构债，归类为方法包结构债，不是产品 UI 验证失败。

## Reduce Chaos Mistake Similar Practice Action

本地 Reduce Chaos guidance 继续推进第 7.2 节：每条 mistake 的按钮应包含 `做一道同类题`。`src/server/mistakes/view.ts` 新增 `buildMistakeSimilarPracticeHref()`，`src/app/mistakes/page.tsx` 在聚焦 sticky 操作区和误区清单动作区接入 `做一道同类题`，链接到 `/today?mode=remediation&source=mistake...`；`src/server/learning/today-remediation-intent.ts` 和 `TodayRemediationBanner` 支持 Mistake 来源落点。本切片只改读侧链接、Today remediation 可见落点、源码级测试和文档记录，不触碰数据库迁移、Preview 写保护、写入 action、Review 评分、Current Mission 排序或生产配置。

- [√] 读取 `roky_learn_reduce_chaos_and_book_companion_guidance.md`，确认当前切片对应第 7.2：错题修复按钮应包含 `做一道同类题`。
- [√] RED：扩展 `tests/unit/mistakes-view.test.ts`，先失败于缺少 `buildMistakeSimilarPracticeHref()`、`做一道同类题` 和页面接线。
- [√] RED：扩展 `tests/unit/today-remediation-intent.test.ts`，先失败于 `source=mistake` 返回 null 和 banner 缺少 `Mistake 同类题短练习`。
- [√] GREEN：Mistakes 聚焦 sticky 操作区和误区清单都新增 `做一道同类题`。
- [√] GREEN：Today remediation 接受 `source=mistake`，显示 `Mistake 同类题短练习`、`错题修复中心`、`生成同类题短练习`，返回 `/mistakes`。
- [√] 运行定向 RED/GREEN。
- [√] 运行相关回归。
- [√] 同步 `docs/ui-review-checklist.md`、`helloagents/modules/mistakes.md`、`helloagents/modules/today-focus-mode.md`、`helloagents/CHANGELOG.md` 初始记录。
- [√] 运行覆盖扫描。
- [√] 运行本地完整门禁。
- [√] 运行 Aegis helper 结构检查并记录既有 Markdown-only 结构债。
- [ ] 提交并推送 GitHub。
...
从 `ai-learning-platform` 根目录继续。当前切片是 `0.372.0 Reduce Chaos Mistake Similar Practice Action`：只改 `/mistakes` 的 `做一道同类题` 读侧入口、`/today?mode=remediation&source=mistake...` 可见落点、源码级测试和文档；不要改数据库 schema/migration、`generateMistakeReviewCardAction()`、`markMistakeResolvedAction()`、Review 评分、Current Mission 排序、Preview 写保护、生产 env/provider 密钥或生产部署。已完成 RED/GREEN、覆盖扫描、相关回归、完整本地门禁、Aegis helper 结构检查和文档同步；下一步提交推送。

## Drift Check

- Scope：仍服务 Roky Learn reduce-chaos 主线，把 `/mistakes` 从解释/制卡/解决推进为包含同类题短练习的完整修复动作。
- Compatibility：不新增迁移，不触碰 Preview Mode 只读边界、写入 action、Review 评分、Current Mission 排序、数据库 schema 或生产配置；只新增读侧 href 和 Today remediation 展示分支。
- New fallback/owner：未新增外部 owner、adapter 或兼容 fallback；Mistake 同类题入口 owner 为 `src/server/mistakes/view.ts`、`src/app/mistakes/page.tsx` 和 `src/server/learning/today-remediation-intent.ts`。
- Retirement：旧的 `/mistakes` 按钮清单缺少指导文件要求的 `做一道同类题`，用户只能解释、制卡或标记解决；该断点由 Mistakes → Today 的同类题短练习入口退役。
- Decision：continue; Reduce Chaos Mistake Similar Practice Action 已完成 RED/GREEN、覆盖扫描、相关回归、完整本地门禁和 Aegis helper 结构检查。下一步提交和推送；生产部署、生产 smoke、数据库迁移和写入型生产 smoke 保持未覆盖边界。Aegis helper 仍失败于既有 Markdown-only 结构债，归类为方法包结构债，不是产品 UI 验证失败。
