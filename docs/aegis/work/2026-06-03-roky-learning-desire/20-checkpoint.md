...
从 `ai-learning-platform` 根目录继续。当前切片是 `0.371.0 Reduce Chaos Mistakes Repair Workflow Progress`：只改 `/mistakes` 读侧修复流程展示、现有 `Flashcard.reviewCount` 查询、源码级测试和文档；不要改数据库 schema/migration、`generateMistakeReviewCardAction()`、`markMistakeResolvedAction()`、Review 评分、Current Mission 排序、Preview 写保护、生产 env/provider 密钥或生产部署。已完成 RED/GREEN、覆盖扫描、相关回归、本地完整门禁、Aegis helper 结构检查和文档同步；下一步提交推送。
...
- Decision：continue; Reduce Chaos Mistakes Repair Workflow Progress 已完成 RED/GREEN、覆盖扫描、相关回归、完整本地门禁和 Aegis helper 结构检查。下一步提交和推送；生产部署、生产 smoke、数据库迁移和写入型生产 smoke 保持未覆盖边界。Aegis helper 仍失败于既有 Markdown-only 结构债，归类为方法包结构债，不是产品 UI 验证失败。
